...
public class PriorityExample {
  ...
  public void run() throws Exception {
    ...
    // priority 值越大，优先级越高
    // 在只有一个线程的情况下，第一次执行的情况是：
    // Priority10Trigger15SecondRepeat -> Priority5Trigger10SecondRepeat -> Priority1Trigger5SecondRepeat
    // 第二次执行由于时间间隔的影响，执行情况是：
    // Priority1Trigger5SecondRepeat -> Priority5Trigger10SecondRepeat -> Priority10Trigger15SecondRepeat
    // 所以总的执行顺序如上面的注释所示

    // Calculate the start time of all triggers as 5 seconds from now
    Date startTime = futureDate(5, IntervalUnit.SECOND);

    // Priority1Trigger5SecondRepeat
    ...
    // Priority5Trigger10SecondRepeat
